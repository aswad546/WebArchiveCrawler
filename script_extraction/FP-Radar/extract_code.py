...
def insert_into_wayback_db(conn, rank, url, wayback_url, code, fp_type, archive):
    """
    Inserts the Wayback JavaScript data into the PostgreSQL database.
    """
    # Remove NUL characters from code
    code = code.replace('\x00', '')
    with conn.cursor() as cur:
        cur.execute("""
            INSERT INTO wayback_fp_data (rank, url, wayback_url, code, fp_type, archive) 
            VALUES (%s, %s, %s, %s, %s, %s);
        """, (rank, url, wayback_url, code, fp_type, archive))
        conn.commit()
    ...
    with tqdm(total=len(extracted_data)) as progress_bar:
        for data in extracted_data:
            ...
            if wayback_url:
                ...
                wayback_url = url  # Mark the live URL as the URL fetched from
                error = "Regular_fetch_error:" + error if error else None

            if code:
                # Insert into the PostgreSQL database
                insert_into_wayback_db(conn, rank, url, wayback_url, code, api_type, archive)
            else:
...
def main():
    ...
    extracted_data = process_json_files("/home/vagrant/WebArchiveCrawler/datasets/FP-Radar")
